...
{
	if (arg[0] == ENV_VAR_IDENTIFIER) {
		if (arg[1] == MAGICAL_VAR) {
			sprintf(arg, "%d", status);
			return arg;
		}

		char *environ_var = getenv(arg + 1);
		if (environ_var) {
			int length = strlen(environ_var);
			if (length > ARGSIZE) {
				arg = realloc(arg, length);
				if (!arg) {
					perror("Error reallocating");
				}
			}
			strcpy(arg, environ_var);
